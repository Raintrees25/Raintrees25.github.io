...
FaviconURL		= 'https://www.pngitem.com/pimgs/m/14-148388_twitch-pepe-the-frog-youtube-video-game-pepe.png';

ChannelName		= 'Pepechat';

MiniLogoURL		= '';

CustomWelcomeText	= 'Welcome to Pepechat!';
...
JoinMessage		= 'hopped on';

LeaveMessage		= 'hopped off';
...
if (typeof CHANNELPATH === "undefined") CHANNELPATH = 'n';
...
_SHOWELEMENTS	= '{"header":1,"logo":1,"motd":1,"announcements":1,"mainheader":1,"playerlabels":1,"chatlabels":1,'
		...
		+ '"mediadbbtns":1,"chatcontrols":1,"pollemotebtns":1,"chatbtns":1,'
...
TTSLANGUAGE	= getOrDefault('SP_ttslanguage',	'it');
...
VISIBLETAB	= {"chats":CHANNEL.name, "commands":1, "emotes":1, "lastmedia":1,
		  "mentions":1, "messages":1, "options":1, "tools":1}
...
function showImagesOnChat(elem) {
	elem.find(ImageExtensions).each(function() {
		...
		var img = $('<img class="embedimg" title="Click to open in a new tab" />').attr('src', 'https://external-content.duckduckgo.com/iu/?u=' + this.href)
		  .load(function() {
			if (SCROLLCHAT) scrollChat();
		  });
...
var html = '<button id="plr-btn" class="btn btn-sm btn-default dropdown-toggle" data-toggle="dropdown" '
	 ...
	 +       'Vol: <span id="volume-lvl">100</span>%</button>'
...
$(document).on('keydown', function(e) {
	if (e.altKey && !e.ctrlKey) {
		...
		else if (String.fromCharCode(e.which) === "4") document.getElementById("layout-1").click()
